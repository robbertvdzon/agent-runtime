...
if [[ -f "$target" ]]; then
  ...
  [[ -n "$(value_for "$target" AR_PERSONAL_NEWS_FEED_TOKEN)" ]] || printf 'AR_PERSONAL_NEWS_FEED_TOKEN=%s\n' "$(random_secret)" >> "$tmp"
  [[ -n "$(value_for "$target" AR_TEST_CONTROL_TOKEN)" ]] || printf 'AR_TEST_CONTROL_TOKEN=%s\n' "$(random_secret)" >> "$tmp"
...
chmod 600 "$tmp"
{
  ...
  printf 'AR_PERSONAL_NEWS_FEED_TOKEN=%s\n' "$(random_secret)"
  printf 'AR_WORKER_TOKEN=%s\n' "$(random_secret)"
  printf 'AR_ADMIN_TOKEN=%s\n' "$(random_secret)"
  printf 'AR_TEST_CONTROL_TOKEN=%s\n' "$(random_secret)"
