...
class V2JobController(private val service:V2JobService,private val jobs:V2JobStore,private val objects:V2ObjectStore,private val usage:V2UsageStore,private val download:V2Download) {
    ...
    @GetMapping("/{jobId}/result") fun result(@PathVariable jobId:String,request:HttpServletRequest):JobResultView {val job=visible(request,jobId);val available=job.view.status==JobStatus.SUCCEEDED||(job.view.status==JobStatus.FAILED&&job.verificationResult!=null);if(!available||job.result==null)throw ApiException("RESULT_NOT_READY","Job has no completed result.",HttpStatus.CONFLICT);val artifacts=objects.outputObjects(jobId).map{(name,o)->OutputObjectView(o.view.objectId,name,o.view.filename,o.view.mimeType,o.view.sizeBytes,o.view.sha256,o.view.state,o.view.createdAt,o.view.readyAt!!,"/v2/jobs/$jobId/objects/${o.view.objectId}/content")};return JobResultView(jobId,job.result,job.repositoryResult,job.verificationResult,artifacts,usage.jobSummary(jobId),job.view.completedAt!!)}
...
class V2WorkerController(private val service:V2WorkerService,private val download:V2Download) {
    ...
    @PostMapping("/{workerId}/jobs/{jobId}/attempts/{attemptId}/repository-publication") fun preparePublication(@PathVariable workerId:String,@PathVariable jobId:String,@PathVariable attemptId:String,@Valid @RequestBody body:PrepareRepositoryPublicationRequest,request:HttpServletRequest):ResponseEntity<Any>{request.requireWorker();val(intent,rejection)=service.preparePublication(workerId,jobId,attemptId,body);return if(rejection==null)ResponseEntity.ok(intent) else ResponseEntity.unprocessableEntity().body(rejection)}
    @PostMapping("/{workerId}/jobs/{jobId}/attempts/{attemptId}/repository-publication/confirm") fun confirmPublication(@PathVariable workerId:String,@PathVariable jobId:String,@PathVariable attemptId:String,@Valid @RequestBody body:ConfirmRepositoryPublicationRequest,request:HttpServletRequest):ResponseEntity<Void>{request.requireWorker();service.confirmPublication(workerId,jobId,attemptId,body);return ResponseEntity.noContent().build()}
    @PostMapping("/{workerId}/jobs/{jobId}/attempts/{attemptId}/repository-publication/discard") fun discardPublication(@PathVariable workerId:String,@PathVariable jobId:String,@PathVariable attemptId:String,@Valid @RequestBody body:DiscardRepositoryPublicationRequest,request:HttpServletRequest):ResponseEntity<Void>{request.requireWorker();service.discardPublication(workerId,jobId,attemptId,body);return ResponseEntity.noContent().build()}
