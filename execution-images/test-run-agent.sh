...
mkdir -p "$temporary/source/nested" "$temporary/source/ipc" "$temporary/target"
printf 'credential\n' > "$temporary/source/nested/auth.json"
...
test "$(cat "$temporary/target/nested/auth.json")" = credential
